(function ($) {
   ...
   */
  Drupal.ajax_facets_custom = {};
  // Store current query value.
  Drupal.ajax_facets_custom.queryState = null;
  // State of each facet.
  Drupal.ajax_facets_custom.facetQueryState = null;
  // Determine if it is first page load. Will be reset in Drupal.ajax_facets_custom.initHistoryState.
  Drupal.ajax_facets_custom.firstLoad = true;
  // Settings of each ajax facet.
  Drupal.ajax_facets_custom.facetsSettings = {};
  // Buttons for ajax facets.
  Drupal.ajax_facets_custom.ajax_facets_custom_buttons = false;
  // Force update of the results
  Drupal.ajax_facets_custom.force_update_results = false;

  // You can use it for freeze facet form elements while ajax is processing.
  Drupal.ajax_facets_custom.beforeAjaxCallbacks = {};

  // You can use it to make actions after the ajax has updated the content.
  Drupal.ajax_facets_custom.afterContentUpdateCallbacks = {};

  Drupal.ajax_facets_custom.beforeAjax = function () {
    $.each(Drupal.ajax_facets_custom.beforeAjaxCallbacks, function () {
      if ($.isFunction(this)) {
        this();
      }
    });
  ...
  Drupal.ajax_facets_custom.afterContentUpdate = function (ajax, response) {
    Drupal.ajax_facets_custom.force_update_results = false;
    $.each(Drupal.ajax_facets_custom.afterContentUpdateCallbacks, function () {
      if ($.isFunction(this)) {
        this(ajax, response);
      }
    });
  ...
  Drupal.behaviors.ajax_facets_custom = {
    attach: function (context, settings) {
      if (!Drupal.ajax_facets_custom.queryState) {
        // Add property for views and set data about facet values.
        Drupal.ajax_facets_custom.queryState = {
          'views': new Array(),
        ...
        if (settings.facetapi.searchPath) {
          Drupal.ajax_facets_custom.queryState['searchPath'] = settings.facetapi.searchPath;
        }
        if (settings.facetapi.index_id) {
          Drupal.ajax_facets_custom.queryState['index_id'] = settings.facetapi.index_id;
        }
        // Check view name and display name for all views.
        if (settings.facetapi.views) {
          $.each(settings.facetapi.views, function(key, view) {
            Drupal.ajax_facets_custom.queryState['views'][key] = {};
            // Get view dom id.
            var viewDomId = Drupal.ajax_facets_custom.getViewDomId(settings, key);
            if (viewDomId) {
              // Take settings from view if it works by AJAX.
              if (Drupal.views && Drupal.views.instances['views_dom_id:' + viewDomId]) {
                $.extend(Drupal.ajax_facets_custom.queryState['views'][key], Drupal.views.instances['views_dom_id:' + viewDomId].settings);
              }
              // If this view doesn't use ajax.
                ...
                if (settings.facetapi.view_args[nameDisplay]) {
                  Drupal.ajax_facets_custom.queryState['views'][key]['view_args'] = settings.facetapi.view_args[nameDisplay];
                }
                // Respect view path.
                if (settings.facetapi.view_path[nameDisplay]) {
                  Drupal.ajax_facets_custom.queryState['views'][key]['view_path'] = settings.facetapi.view_path[nameDisplay];
                }
              }
            }
            ...
            if (view.view_name && view.view_display_id) {
              ...
              if (settings.facetapi.exposed_input[nameDisplay]) {
                $.extend(Drupal.ajax_facets_custom.queryState, settings.facetapi.exposed_input[nameDisplay]);
              }
            }
          });
        }
        ...
        if (settings.facetapi.facet_field) {
          Drupal.ajax_facets_custom.queryState['facet_field'] = settings.facetapi.facet_field;
        }
      }
      ...
      if (settings.facetapi) {
        for (var index in settings.facetapi.facets) {
          Drupal.ajax_facets_custom.bindResetLink(settings.facetapi.facets[index].id, index, settings);
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_checkboxes') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper input.facet-multiselect-checkbox:not(.processed)').change(
              ...
              Drupal.ajax_facets_custom.processCheckboxes
            ).addClass('processed');
          }
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_select') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper select:not(.processed)').each(function () {
              $(this).change(
                [settings.facetapi.facets[index]],
                Drupal.ajax_facets_custom.processSelectbox
              ).addClass('processed');
            });
          }
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_links') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper a:not(.processed)').each(function () {
              $(this).click(
                [settings.facetapi.facets[index]],
                Drupal.ajax_facets_custom.processLink
              ).addClass('processed');
            });
          }
            ...
            $('#' + settings.facetapi.facets[index].id + '-wrapper div.slider-wrapper:not(.processed)').each(function () {
              ...
              $sliderWrapper.slider({
                ...
                stop: function( event, ui ) {
                  Drupal.ajax_facets_custom.processSlider($sliderWrapper, ui.values[0], ui.values[1]);
                }
              }).addClass('processed');
              ...
              $sliderWrapperParent.find("input[type='text']").each(function() {
                $(this).change(function() {
                  ...
                  if (isFinite(min) && isFinite(max) && min < max) {
                    ...
                    Drupal.ajax_facets_custom.processSlider($sliderWrapper, min, max);
                  }
                });
              });
          ...
          Drupal.ajax_facets_custom.facetsSettings[settings.facetapi.facets[index].facetName] = settings.facetapi.facets[index];
        }
      }

      // Hide blocks with ajax-facets-empty-behavior.
      $('.ajax-facets-empty-behavior').parents('.block--ajax_facets_custom').hide();
    }
  };

  Drupal.behaviors.ajax_facets_custom_block = {
    attach: function (context, settings) {
      // Reset all link.
      $('.ajax-facets-reset-all-link:not(.processed)').bind('click', function (e) {
        ...
        Drupal.ajax_facets_custom.queryState.f = [];

        // Send ajax query with first found facet.
        Drupal.ajax_facets_custom.force_update_results = true;
        Drupal.ajax_facets_custom.sendAjaxQuery({
      ...
      if ($submitLink.length) {
        // Submit link found.
        Drupal.ajax_facets_custom.ajax_facets_custom_buttons = true;
        // Submit all link.
  ...
  Drupal.ajax_facets_custom.bindResetLink = function (facetWrapperId, index, settings) {
    var facet_values = Drupal.ajax_facets_custom.getFacetValues();
    ...
    $facetWrapper.find('a.reset-link:not(".processed")').addClass('processed').click(function (event) {
      ...
      Drupal.ajax_facets_custom.excludeCurrentFacet(facetName);
      Drupal.ajax_facets_custom.sendAjaxQuery({
        pushStateNeeded: true,
        searchResultsNeeded: true
      });
   ...
   */
  Drupal.ajax_facets_custom.processSelectbox = function (event) {
    var $this = $(this);
    var facetName = $this.data('facet-name');
    // Init history.
    Drupal.ajax_facets_custom.initHistoryState($this);
    // If facets are already defined in queryState.
    if (Drupal.ajax_facets_custom.queryState['f']) {
      // Exclude all values for this facet from query.
      Drupal.ajax_facets_custom.excludeCurrentFacet(facetName);

      /* Default value. */
      if ($this.find(":selected").val() == '_none') {
        delete Drupal.ajax_facets_custom.queryState['f'][Drupal.ajax_facets_custom.queryState['f'].length];
      } else {
        Drupal.ajax_facets_custom.queryState['f'][Drupal.ajax_facets_custom.queryState['f'].length] = facetName + ':' + $this.find(":selected").val();
      }
    }

    Drupal.ajax_facets_custom.sendAjaxQuery({
      pushStateNeeded: !Drupal.ajax_facets_custom.ajax_facets_custom_buttons,
      searchResultsNeeded: !Drupal.ajax_facets_custom.ajax_facets_custom_buttons
    });
  };
   ...
   */
  Drupal.ajax_facets_custom.processCheckboxes = function (event) {
    var $this = $(this);
    var facetName = $this.data('facet-name');
    ...
    if (Drupal.ajax_facets_custom.queryState['f']) {
      ...
      if ($this.is(':checked')) {
        ...
        for (var index in Drupal.ajax_facets_custom.queryState['f']) {
          // If we already have this value in queryState.
          if (Drupal.ajax_facets_custom.queryState['f'][index] == name_value) {
        ...
        if (addCurrentParam) {
          ...
          if (Drupal.ajax_facets_custom.facetsSettings[rawFacetName].limit_active_items) {
            Drupal.ajax_facets_custom.excludeCurrentFacet(facetName);
          }
          Drupal.ajax_facets_custom.queryState['f'][Drupal.ajax_facets_custom.queryState['f'].length] = name_value;
        }
      }
      // If we unset filter, remove it from query.
        ...
        for (var index in Drupal.ajax_facets_custom.queryState['f']) {
          if (Drupal.ajax_facets_custom.queryState['f'][index] != name_value) {
            queryNew[queryNew.length] = Drupal.ajax_facets_custom.queryState['f'][index];
          }
        }
        Drupal.ajax_facets_custom.queryState['f'] = queryNew;
      }
    }
   ...
   */
  Drupal.ajax_facets_custom.processLink = function (event) {
    var $this = $(this);
    var facetName = $this.data('facet-name');
    var name_value = facetName + ':' + $this.attr('data-facet-value');// $.data can round decimal values like 4.0 to 4, avoid it.
    var rawFacetName = $this.data('raw-facet-name');
   ...
   */
  Drupal.ajax_facets_custom.processSlider = function($sliderWrapper, min, max) {
    var facetName = $sliderWrapper.data('facet-name');
    // Init history.
    Drupal.ajax_facets_custom.initHistoryState($sliderWrapper);
    // If facets are already defined in queryState.
    if (Drupal.ajax_facets_custom.queryState['f']) {
      // Exclude all values for this facet from query.
      Drupal.ajax_facets_custom.excludeCurrentFacet(facetName);
      Drupal.ajax_facets_custom.queryState['f'][Drupal.ajax_facets_custom.queryState['f'].length] = facetName + ':[' + min + ' TO ' + max + ']';
    }
   ...
   * Exclude all values for this facet from query.
   */
  Drupal.ajax_facets_custom.excludeCurrentFacet = function (facetName) {
    facetName = facetName + ':';
    var queryNew = [];
    for (var index in Drupal.ajax_facets_custom.queryState['f']) {
      if (Drupal.ajax_facets_custom.queryState['f'][index].substring(0, facetName.length) != facetName) {
        queryNew[queryNew.length] = Drupal.ajax_facets_custom.queryState['f'][index];
      }
    }
    Drupal.ajax_facets_custom.queryState['f'] = queryNew;
   ...
   * Send ajax.
   */
  Drupal.ajax_facets_custom.sendAjaxQuery = function (options) {
    Drupal.ajax_facets_custom.beforeAjax();
    var data = Drupal.ajax_facets_custom.queryState;
    // Render the exposed filter data to send along with the ajax request
    $.each(Drupal.ajax_facets_custom.queryState.views, function (key, view) {
    ...
    var settings = {
      url: encodeURI(Drupal.settings.basePath + Drupal.settings.pathPrefix + 'ajax/ajax_facets_custom/refresh'),
      submit: {'ajax_facets_custom': data}
    };
    var ajax = new Drupal.ajax(false, false, settings);
    ajax.success = function (response, status) {
      // Push new state only on successful ajax response.
      if (options.pushStateNeeded) {
        Drupal.ajax_facets_custom.pushState();
      }
  ...
  Drupal.ajax_facets_custom.getFacetValues = function () {
    var f = Drupal.ajax_facets_custom.queryState.f;
    var facets_values = {};
    var symbol = ':';
  ...
  Drupal.ajax_facets_custom.getViewDomId = function(settings, key) {
   ...
   */
  Drupal.ajax_facets_custom.getFacetsQueryUrl = function (baseUrl) {
    var query = {
      'f': []
    };

    // Clone variable.
    $.extend(true, query.f, Drupal.ajax_facets_custom.queryState.f);
    ...
    // like Bing or Google. So we just bring this behaviour to ajax_facets_custom module.
    ...
    if (Drupal.ajax_facets_custom.queryState.query) {
      query.query = Drupal.ajax_facets_custom.queryState.query;
    }

    if (Drupal.ajax_facets_custom.queryState.order) {
      query.order = Drupal.ajax_facets_custom.queryState.order;
      query.sort = Drupal.ajax_facets_custom.queryState.sort;
    }

    if (Drupal.ajax_facets_custom.queryState.pages) {
      query.pages = Drupal.ajax_facets_custom.queryState.pages;
    }

    // Respect existing query parameters in url.
    // Merge respected parameters with facet parameters recursively without duplicates.
    $.extend(true, query, Drupal.ajax_facets_custom.simplifyObject(Drupal.ajax_facets_custom.getAdditionalQueryParameters()));
   ...
   */
  Drupal.ajax_facets_custom.getAdditionalQueryParameters = function () {
    var result = {};
   ...
   */
  Drupal.ajax_facets_custom.simplifyObject = function (obj) {
    for (var name in obj) {
   ...
   * handler.
   */
  Drupal.ajax_facets_custom.initHistoryState = function ($facet) {
    // Set the initial state only initial page load.
    if (Drupal.ajax_facets_custom.firstLoad) {
      Drupal.ajax_facets_custom.firstLoad = false;

      // If history.js available - use it.
      if (Drupal.settings.facetapi.isHistoryJsExists) {
        History.replaceState({
          facets: Drupal.ajax_facets_custom.queryState['f']
        }, null, null);
      } else {
   ...
   */
  Drupal.ajax_facets_custom.pushState = function () {
    var stateUrl = Drupal.ajax_facets_custom.getFacetsQueryUrl(Drupal.settings.facetapi.searchUrl),
      state = {
        facets: Drupal.ajax_facets_custom.queryState['f']
      },
    ...
    if (Drupal.settings.facetapi.isHistoryJsExists) {
      ...
      $window.unbind('statechange', Drupal.ajax_facets_custom.reactOnStateChange);
      History.pushState(state, title, stateUrl);
      $window.bind('statechange', Drupal.ajax_facets_custom.reactOnStateChange);
    } else {
      // Fallback to HTML5 history object.
   ...
   */
  Drupal.ajax_facets_custom.reactOnStateChange = function () {
    var state = null,
      facets = [];
    ...
    if (window.location.pathname == Drupal.settings.facetapi.searchUrl && facets) {
      Drupal.ajax_facets_custom.queryState['f'] = facets;
      Drupal.ajax_facets_custom.force_update_results = true;
      Drupal.ajax_facets_custom.sendAjaxQuery({
  ...
  // All farther work does Drupal.ajax_facets_custom.pushState() function.
  // If history.js Adapter available - use it to bind "statechange" event.
  if (History.Adapter && Drupal.ajax_facets_custom.firstLoad) {
    History.Adapter.bind(window, 'statechange', Drupal.ajax_facets_custom.reactOnStateChange);
  } else {
    // Fallback to default HTML5 event.
    window.onpopstate = function () {
      Drupal.ajax_facets_custom.reactOnStateChange();
    };
  }

  if (Drupal.ajax) {
    // Command for process search results and facets by ajax.
    Drupal.ajax.prototype.commands.ajax_facets_custom_update_content = function(ajax, response) {
      if (response.data.activeItems) {
        Drupal.ajax_facets_custom.facetQueryState = response.data.activeItems;
      }
      ...
      if (response.data.newContent) {
        for (var id in response.data.newContent) {
          ...
          var $block = $('#' + id + '-wrapper').parents('div.block--ajax_facets_custom:not(:visible)');
          if ($block.size()) {
            $block.show();
          }
        }
      ...
      var buttons_exist = Drupal.ajax_facets_custom.ajax_facets_custom_buttons,
        force_update_results = Drupal.ajax_facets_custom.force_update_results;
      ...
      Drupal.ajax_facets_custom.afterContentUpdate(ajax, response);
